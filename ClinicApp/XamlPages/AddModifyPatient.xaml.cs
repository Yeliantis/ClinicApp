    ...
    public partial class AddModifyPatient : Page
    {
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Сравнение введенной даты с актуальной, получаемой сервисом
                int compare = DateTime.Compare((DateTime)dateOfBirth, await ActualDataGetter.GetDate());
                ...
                #endregion
                //Проверка на возраст пациента (не больше 110 лет)
                if (!await DateValidation.dateIsValid((DateTime)dateOfBirth))
                {
                    status = false;
                    messageBuilder.Append("Введена некорректная дата");
                }
                if (compare > 0)
                {
                    status = false;
                    messageBuilder.Append("Пациент еще не родился");
                }
